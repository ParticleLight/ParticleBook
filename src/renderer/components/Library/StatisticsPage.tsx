  ...
  return (
    <div className="flex items-center gap-3 py-2">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs truncate" style={{ color: 'var(--text-tertiary)' }}>{book.author || t('未知作者')}</div>
      ...
      <div className="text-right flex-shrink-0 w-24">
        ...
        <div className="text-xs" style={{ color: 'var(--text-tertiary)' }}>{readingTime > 0 ? formatReadingTime(readingTime) : t('未阅读')}</div>
      </div>
    </div>
  )
}
    ...
    <div className="h-full flex flex-col" style={{ background: 'var(--bg)' }}>
      ...
      <div className="flex items-center gap-3 px-6 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>{t('阅读统计')}</span>
        </div>
      </div>

      {/* Summary */}
      <div className="grid grid-cols-3 gap-4 px-6 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
        <div className="text-center"><div className="text-2xl font-bold" style={{ color: 'var(--accent)' }}>{allBooks.length}</div><div className="text-xs mt-1" style={{ color: 'var(--text-tertiary)' }}>{t('总书籍')}</div></div>
        <div className="text-center"><div className="text-2xl font-bold" style={{ color: 'var(--color-green)' }}>{formatReadingTime(totalTime)}</div><div className="text-xs mt-1" style={{ color: 'var(--text-tertiary)' }}>{t('总阅读时间')}</div></div>
        <div className="text-center"><div className="text-2xl font-bold" style={{ color: 'var(--color-yellow)' }}>{booksWithProgress}</div><div className="text-xs mt-1" style={{ color: 'var(--text-tertiary)' }}>{t('已开始阅读')}</div></div>
      </div>

      {/* Book list */}
      <div className="flex-1 overflow-y-auto px-6 py-3">
        {bookStats.length === 0 ? <div className="text-center py-8" style={{ color: 'var(--text-tertiary)' }}>{t('暂无书籍')}</div> : <div className="space-y-3">{bookStats.map((item) => <BookRow key={item.book.id} book={item.book} readingTime={item.readingTime} progress={item.progress} />)}</div>}
      </div>
    </div>
  )
}
